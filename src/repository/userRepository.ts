...
class UserRepository {
    ...
    async login(data: userTypes) {
        try {
            ...
            } else { // if user give password and email for login in system 
            ...
            console.error('Error at user repo login level:', error);
    ...
    async refershAccessToken(incomingRefreshToken: string) {
        try {
            ...
            // verfiy by jwt 
            const decodedData = jwt.verify(incomingRefreshToken, serverConfigVariable.ACCESS_TOKEN_SECRET) as decodedDataTypes
    ...
    async changePassword(data: changePasswordTypes) {
        ...
        try {
            if (
                [oldPassword, newPassword, confiromPassword].some((value) => value !== "")
            ) {
